<template>
  <nav class="bt-glass pointer-events-auto flex w-full items-center justify-between p-1 text-sm shadow-sm sm:rounded-xl sm:p-2 md:w-auto">
    <section class="flex items-center justify-between">
      ...
      <MainToolbarButton
        :active="mainToolbarStore.currentToolbar === 'measurements'"
      ...
      <MainToolbarButton
        :active="mainToolbarStore.currentToolbar === 'draw'"
      ...
      <MainToolbarButton
        :title="selectedCount > 1 ? `Bulk Edit (${selectedCount})` : 'Bulk Edit'"
      ...
      >
        <div class="flex items-center gap-2">
          ...
          <button class="qb-btn" @click="openDeleteEventsDialog">Delete events in time range…</button>
          <button class="qb-btn" @click="openCopyRangeRingsDialog">Copy range-rings…</button>
          <button class="qb-btn qb-cancel" @click="bulkMenuOpen = false">Close</button>
        </div>
      </FloatingPanel>

      <DeleteEventsBulkDialog
        v-model="deleteEventsDialogOpen"
        :selected-unit-ids="selectedUnitIdsArray"
      />

      <CopyRangeRingsBulkDialog
        v-model="copyRangeRingsDialogOpen"
        :selected-unit-ids="selectedUnitIdsArray"
      />
      ...
      <div class="ml-2 flex items-center">
        <EchelonPickerPopover :symbol-options="symbolOptions" :select-echelon="selectEchelon" />
    ...
    <section class="flex items-center">
      ...
      <MainToolbarButton title="Previous Day" class="hidden sm:flex" @click="emit('dec-day')">
      ...
      <MainToolbarButton title="Previous Event" class="hidden sm:flex" @click="emit('prev-event')">
        <span class="sr-only">Previous Event</span>
        <IconSkipPrevious class="size-5" aria-hidden="true" />
      </MainToolbarButton>

      <MainToolbarButton title="Next Event" class="hidden sm:flex" @click="emit('next-event')">
    ...
    >
      Click on globe or ORBAT to place unit.
      <Button type="button" variant="link" size="sm" @click="cancelGetLocation()">Cancel</Button>
    </FloatingPanel>
  </nav>
</template>
...
const { store: scenarioStore, unitActions, geo, helpers } = injectStrict(activeScenarioKey);
...
const { undo, redo, canRedo, canUndo, groupUpdate } = scenarioStore;
...
const DEFAULT_NEW_UNIT_ALT_M = 0;
...
function startGetLocation() {
  ...
  props.requestGlobeLocation((location) => {
    ...
    groupUpdate(() => {
      ...
      if (unitId) addUnitPosition(unitId, withDefaultAlt(location));
    });
...
function cancelGetLocation() {
  isGetLocationActive.value = false;
  selectStore.hoverEnabled = true;
  try { props.cancelGlobeLocation(); } catch {}
}
