<script setup lang="ts">
    ...
    const emit = defineEmits<{
        (e: "event-click", ev: NScenarioEvent): void;
    }>();
    ...
    function onEventClick(event: NScenarioEvent) {
        // drive the 3D dock selection
        (activeScenarioEventId as any).value = event.id;
    ...
    function addEvent() {
        ...
        (activeScenarioEventId as any).value = eventId;
    }
...
<template>
    <div class="p-1">
        ...
        <Button v-if="!selectOnly"
                size="sm"
                variant="outline"
                @click="addEvent()"
                class="mt-3 text-gray-200 border-white/20 bg-white/0 hover:bg-white/5 hover:text-white">
            Add scenario event
        </Button>
    </div>
</template>
