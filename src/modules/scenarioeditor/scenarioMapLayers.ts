...
import { FirmsLayerController } from "@/modules/scenarioeditor/FIRMSLayer/firmsLayerController";

const layersMap = new WeakMap<OLMap, LayerGroup>();

// Responsible for creating and managing the OpenLayers layers for the scenario editor map-layer stack
export function useScenarioMapLayers(olMap: OLMap) {
    ...
    const firmsControllers = new Map<FeatureId, FirmsLayerController>();
    let lastScenarioTime = Number.NaN;
    ...
    function initializeFromStore() {
        // cleanup FIRMS controllers
        firmsControllers.forEach((c) => c.dispose());
        firmsControllers.clear();
    ...
    function addFIRMSLayer(data: ScenarioFIRMSLayer) {
        const source = new VectorSource();
        ...
        // create controller
        const controller = new FirmsLayerController({
            olMap,
            vectorSource: source,
            layer: data,
            onStatus: (s) => {
            ...
            maxCachedDays: 90, // conservative default; adjust later if desired
        });

        firmsControllers.set(data.id, controller);

        // set initialized state for UI once config is minimally viable; controller will override status as it works
        scn.geo.updateMapLayer(
            data.id,
            { _status: data.mapKey ? "loading" : "uninitialized" },
            { noEmit: true, undoable: false },
        );

        mapLayersGroup.getLayers().push(newLayer);

        if (Number.isFinite(lastScenarioTime)) {
            void controller.refresh(lastScenarioTime);
        }
    }

    function deleteLayer(layerId: FeatureId) {
        const mapLayer = scn.geo.getMapLayerById(layerId) as ScenarioMapLayer | undefined;
        if (mapLayer?.type === "FIRMSLayer") {
            const c = firmsControllers.get(layerId);
            c?.dispose();
            firmsControllers.delete(layerId);
        }
    ...
    function addLayer(layerId: FeatureId) {
        ...
        if (mapLayer.type === "FIRMSLayer") addFIRMSLayer(mapLayer);
    }

    function updateLayer(layerId: FeatureId, data: ScenarioMapLayerUpdate) {
        const mapLayer = scn.geo.getMapLayerById(layerId) as ScenarioMapLayer;
        ...
        // URL changes for standard tile/image layers re-create the OL layer
        if (
        ...
        if (mapLayer.type === "FIRMSLayer") {
            ...
            c.updateLayer(mapLayer as ScenarioFIRMSLayer);
            if (Number.isFinite(lastScenarioTime)) {
                void c.refresh(lastScenarioTime);
            }
        }
    }
    ...
    // FIRMS time sensitivity: refresh FIRMS controllers when scenario time changes
    watch(
        () => scn.store.state.currentTime,
        (t) => {
            lastScenarioTime = Number(t);
            if (!Number.isFinite(lastScenarioTime)) return;
            firmsControllers.forEach((c) => void c.refresh(lastScenarioTime));
        },
        { immediate: true },
    );
...
export function getMapLayerIcon(mapLayer: ScenarioMapLayer) {
    ...
    if (mapLayer.type === "TileJSONLayer" || mapLayer.type === "XYZLayer")
        return IconWebBox;
    if (mapLayer.type === "FIRMSLayer") return VectorIcon; // safe fallback icon
    return ImageIcon;
}
