...
import { computed, onMounted, onBeforeUnmount, type Ref, watch } from "vue";
...
/* ---------- 2D Map drop target (ORBAT ▶ drop-on-map) ---------- */
/**
 * Enable/disable DnD logs at runtime:
 *   window.__MENTAT_DEBUG_DND = true  // enable
 *   window.__MENTAT_DEBUG_DND = false // disable
 */
const dndDebugEnabled = () => Boolean((window as any).__MENTAT_DEBUG_DND);
function dndLog(label: string, payload?: unknown) {
...
/**
 * Upsert logic for moving units at the current scenario time:
 * - If there is no location strictly prior to current time, treat this as changing the unit baseline (unit.location).
 * - Else replace the state location at current time (do not append), via geo.addUnitPosition for reliable refresh.
 */
    function upsertUnitLocationAtCurrentTime(unitId: string, pos: Position) {
        const cur = Number(state.currentTime);

        const unit: any = (state as any)?.unitMap?.[unitId];
        const st: any[] = Array.isArray(unit?.state) ? unit.state : [];

        // If there's already an entry at the current time, replace it (avoid duplicates)
        const idxAtCur = st.findIndex((e) => e && Number(e.t) === cur);

        const ua: any = unitActions as any;

        if (idxAtCur >= 0) ua.deleteUnitStateEntry?.(unitId, idxAtCur);

        // Always add a time-stamped state location at current scenario time
        addUnitPosition(unitId, pos);
        ua.updateUnitState?.(unitId);
    }


let mapDropCleanup: (() => void) | null = null;
...
watch(
  ...
  (mapMaybe) => {
    ...
    const map = (mapMaybe as unknown as OLMap) ?? null;
    if (!map) return;

    const viewport = map.getViewport?.();
    if (!viewport) return;

    dndLog("🗺️ 2D DROP ▶ installed");

    mapDropCleanup = dropTargetForElements({
      element: viewport,
      canDrop: ({ source }) => isUnitDragItem(source.data),
      onDrop: ({ source, location }) => {
        const input = location.current.input;
        const clientX = (input as any)?.clientX;
        const clientY = (input as any)?.clientY;

        dndLog("🗺️ 2D DROP ▶ onDrop", {
          clientX,
          clientY,
          source: source.data,
        ...
        const pixel = [clientX - rect.left, clientY - rect.top] as [number, number];
        const coords3857 = map.getCoordinateFromPixel(pixel);
        if (!coords3857) return;

        const [lon, lat] = toLonLat(coords3857);
        const pos = [lon, lat] as unknown as Position;

        const unitIds: string[] =
          (source.data as any)?.unitIds ??
          ((source.data as any)?.unit?.id ? [(source.data as any).unit.id] : []);

        if (!unitIds.length) return;

        groupUpdate(() => {
          unitIds.forEach((id) => upsertUnitLocationAtCurrentTime(id, pos));
        });
      },
    });
  },
