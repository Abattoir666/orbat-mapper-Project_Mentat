        ...
        <Dialog as="div" class="relative z-10" @close="open = false">
            ...
            <div class="fixed inset-0 z-10 overflow-y-auto p-4 sm:p-6 md:p-20">
                <TransitionChild as="template"
                    ...
                    <DialogPanel class="ring-opacity-5 mx-auto max-w-xl transform divide-y divide-gray-100 overflow-hidden rounded-xl bg-white shadow-2xl ring-1 ring-black transition-all"
                        ...
                        <Combobox @update:modelValue="onSelect">
                            <div class="relative">
                                <MagnifyingGlassIcon class="pointer-events-none absolute top-3.5 left-4 h-5 w-5 text-gray-400"
                                                     aria-hidden="true" />
                                <ComboboxInput class="h-12 w-full border-0 bg-transparent pr-4 pl-11 text-gray-900 placeholder:text-gray-400 focus:ring-0 sm:text-sm"
                                               placeholder="Search..."
                                               @change="rawQuery = $event.target.value" />
                            </div>
                            <div v-if="isGeoSearch && rawQuery.trim()"
                                 class="px-11 py-1 text-xs text-gray-500">
                                <span v-if="parseGeoQuery(rawQuery)">
                                    {{ parseGeoQuery(rawQuery)?.lat.toFixed(5) }}, {{ parseGeoQuery(rawQuery)?.lon.toFixed(5) }}
                                    <span v-if="parseGeoQuery(rawQuery)?.zoom"> @{{ parseGeoQuery(rawQuery)?.zoom }}</span>
                                </span>
                            </div>
                            <ComboboxOptions v-if="groupedHits && hitCount > 0"
                                             static
                                ...
                                <li v-for="[source, hits] in groupedHits">
                                    ...
                                    <ul class="-mx-4 mt-2 text-sm font-medium text-gray-700">
                                        <ComboboxOption v-for="item in hits"
                                                        :key="item.id"
                                                        :value="item"
                                                        as="template"
                                                        v-slot="{ active }">
                                            <CommandPaletteUnitItem v-if="item.category === 'Units'"
                                                                    :item="item"
                                                                    :active="active"
                                                                    class="" />
                                            <CommandPaletteLayerFeatureItem v-else-if="item.category === 'Features'"
                                                                            :active="active"
                                                                            :item="item" />
                                            <CommandPaletteImageLayerItem v-else-if="item.category === 'Map layers'"
                                                                          :active="active"
                                                                          :item="item" />
                                            <CommandPaletteEventItem v-else-if="item.category === 'Events'"
                                                                     :active="active"
                                                                     :item="item" />
                                            <CommandPalettePlaceItem :item="item"
                                                                     v-else-if="item.category === 'Places'"
                                                                     :active="active"
                                                                     :center="mapCenter" />
                                            <CommandPaletteActionItem v-else-if="item.category === 'Actions'"
                                                                      :active="active"
                                                                      :item="item" />
                                            <CommandPaletteCoordItem v-else-if="item.category === 'Coordinates'"
                                                                     :active="active"
                                                                     :item="item" />
                                            <p v-else>{{ item }}</p>
                                        </ComboboxOption>
                            ...
                            <div v-if="geoDebouncedQuery !== '' && rawQuery !== '?' && hitCount === 0"
...
<script setup lang="ts">
    ...
    import { parseGeoQuery } from "@/utils/parseGeoQuery";
    import { makeZoomHelpers } from "@/composables/useMapZoom";
    ...
    import CommandPaletteCoordItem from "@/components/CommandPaletteCoordItem.vue";
    ...
    // Bind zoom helper to the current OpenLayers map (must be before onSelect)
    ...
    watch(
        ...
        async (q) => {
            ...
            const parsed = parseGeoQuery(q);
            if (parsed) {
                const coordItem = {
                    id: "__coord__",
                    category: "Coordinates" as const,
                    lat: parsed.lat,
                    lon: parsed.lon,
                    zoom: parsed.zoom ?? parsed.impliedZoom, 
                    // shown as the tiny sublabel; tweak as you like:
                    label: `Press Enter to zoom${(parsed.zoom ?? parsed.impliedZoom) ? ` @${parsed.zoom ?? parsed.impliedZoom}` : ""}`
                };
                groupedHits.value = new Map([["Coordinates", [coordItem as any]]]);
                hitCount.value = 1;
                return;
            }

            // fallback: normal geocoder results
    ...
    function onSelect(
        ...
        if ((item as any)?.id === "__coord__") {
            const { lon, lat, zoom } = item as any;
            zoomToLonLat(lon, lat, zoom ?? 12, 300);
            open.value = false;
            return;
        }
        // 🧭 Special case: our synthetic "Zoom to coordinates" action
        if ((item as any)?.id === "__coord__") {
            const { lon, lat, zoom } = item as any;
            const z = zoom ?? 12; // already carried impliedZoom above; 12 is just a final fallback
            zoomToLonLat(lon, lat, z, 300);
            open.value = false;
            return;
        }
