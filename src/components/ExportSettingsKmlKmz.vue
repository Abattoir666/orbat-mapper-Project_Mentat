<script setup lang="ts">
    ...
    type ExtendedExportSettings = ExportSettings & {
        iconScale: number;
        labelScale: number;
    ...
    const props = defineProps<{
        format: ExportFormat;
        modelValue: ExtendedExportSettings;
    }>();

    const form = defineModel<ExtendedExportSettings>({ required: true });
...
<template>
    <fieldset class="space-y-4">
        <div class="grid grid-cols-1 gap-8 sm:grid-cols-2">
            ...
            <InputCheckbox v-if="form.includeUnits"
            ...
            <div v-else />
            <InputCheckbox label="Include scenario features"
                           v-model="form.includeFeatures"
                           description="" />
            <InputCheckbox label="Use short unit names"
                           v-if="isKml || isKmz"
                           v-model="form.useShortName" />
            <InputCheckbox v-if="isKmz"
            ...
            <InputGroupTemplate class="col-span-full" label="Folder settings">
                <MRadioGroup class="mt-4 sm:flex sm:gap-6">
                    ...
                    <InputRadio v-model="form.folderMode" value="sideGroup">Side and groups (nested)</InputRadio>
                </MRadioGroup>
            </InputGroupTemplate>
        </div>
        ...
        <div class="mt-6 grid grid-cols-1 gap-8 sm:grid-cols-2">
            <InputGroupTemplate label="Label scale">
                ...
                <template #description>
                    Setting it to 0 will hide the label
                </template>
                <template #hint>
                    <span class="text-sm font-medium">{{ form.labelScale }}x</span>
                </template>
            </InputGroupTemplate>
            <InputGroupTemplate label="Icon scale">
                <Slider v-model="iconScale" :min="0.5" :max="3" :step="0.1" class="mt-4" />
                <template #description>
                    A scale of 1 is approximately 32x32 pixels
                </template>
                <template #hint>
                    <span class="text-sm font-medium">{{ form.iconScale }}x</span>
                </template>
            </InputGroupTemplate>
        </div>
        <NewAccordionPanel label="Time mode" default-open>
            <MRadioGroup class="mt-2 sm:flex sm:gap-6">
                <InputRadio v-model="form.timeMode" value="current">Current time ({{ formattedTime }})</InputRadio>
            ...
            <div v-else-if="form.timeMode === 'multiple'" class="mt-4 space-y-2">
                ...
                <InputCheckbox v-for="e in events"
                               :label="e.label"
                               :value="e.value"
        ...
        <NewAccordionPanel label="Advanced settings" v-if="isKmz" default-open>
            <div class="grid grid-cols-1 gap-8 sm:grid-cols-2">
                <InputCheckbox label="Draw symbol outline"
                               v-model="form.drawSymbolOutline"
                               description="Improves visibility" />
                <InputCheckbox label="Render symbol amplifiers"
