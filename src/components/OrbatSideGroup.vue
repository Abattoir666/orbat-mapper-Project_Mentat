<script setup lang="ts">
    ...
    let _orbatSideGroupLast = 0;
    function _orbatSideGroupCanLog() {
        if (!ORBAT_DEBUG) return false;
        const now = Date.now();
        if (now - _orbatSideGroupLast < 80) return false;
        _orbatSideGroupLast = now;
        return true;
    }
    function _orbatSideGroupLog(tag: string, data?: any) {
        if (!_orbatSideGroupCanLog()) return;
        try {
            console.log(`🧩 OrbatSideGroup ▶ ${tag}`, data ?? "");
        } catch { }
    }
    ...
    const onSideGroupAction = (group: NSideGroup, action: SideAction) => {
        _orbatSideGroupLog("sidegroup-action", { groupId: group?.id, name: (group as any)?.name, action });
    ...
    const onUnitAction = (unit: NUnit, action: UnitAction) => {
        _orbatSideGroupLog("unit-action passthrough", { unitId: unit?.id, name: (unit as any)?.name, action });
        emit("unit-action", unit, action);
    };
</script>
<template>
    <div>
        <header class="group relative mt-1 flex items-center justify-between py-0"
                :class="isDragging ? 'opacity-20' : ''"
                ref="dropRef"
                :id="`osg-${group.id}`">
            <IconDrag class="h-6 w-6 cursor-move text-gray-400 group-focus-within:opacity-100 group-hover:opacity-100 sm:-ml-3 sm:opacity-0"
                      ref="dragRef" />
            <div class="flex flex-auto items-center">
                <button @click="toggleOpen"
                        class="flex w-full items-center justify-between text-left">
                    <span class="text-sm font-medium text-gray-800 hover:text-gray-900 dark:text-gray-300">
                        {{ group.name || "Units" }}
                    </span>
            ...
            <SideGroupDropdownMenu :is-locked="isLocked"
            ...
            <TreeDropIndicator v-if="instruction"
                               :instruction="instruction"
                               class="z-10 -my-2 -ml-2" />
        </header>
        <EditSideGroupForm v-if="showEditForm"
                           @close="showEditForm = false"
                           :side-group-id="group.id"
                           class="-ml-6" />
        <section v-if="isOpen">
            <OrbatTree :units="group.subUnits"
            ...
            <div v-if="!group.subUnits.length"
