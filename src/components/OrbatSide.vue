<script setup lang="ts">
    ...
    /**
     * 🔎 ORBAT debug logging (Unicode-safe, throttled)
     * Enable:
    ...
    function canLog(): boolean {
        ...
        const now = Date.now();
        if (now - _lastLogMs < 80) return false; // ~12 logs/sec max
        _lastLogMs = now;
        return true;
    }
    function miniUnit(u: any) {
        if (!u) return null;
        if (typeof u === "string") return { id: u };
        if (typeof u !== "object") return { t: typeof u, v: u };
        // wrapper payload { unit: ... }
        if ((u as any).unit && typeof (u as any).unit === "object") {
            const uu = (u as any).unit;
            return { id: uu?.id, name: uu?.name };
        }
        return { id: (u as any).id, name: (u as any).name };
    }
    function log(tag: string, data?: any) {
        if (!canLog()) return;
        try {
            console.log(`🧩 OrbatSide ▶ ${tag}`, data ?? "");
        } catch {
            // ignore
        }
    }
    ...
    onMounted(() => {
        log("mounted", {
            side: { id: props.side.id, name: props.side.name },
            groupCount: props.side.groups?.length ?? 0,
        });

        if (!dropRef.value) {
            log("mounted: dropRef missing", { sideId: props.side.id });
            return;
        }

        // (leave your existing onMounted logic exactly as-is below)
    ...
    onUnmounted(() => {
        log("unmounted", { sideId: props.side.id });
        dndCleanup();
    });
    ...
    const onSideAction = (action: SideAction) => {
        log("side-action", { sideId: props.side.id, action });
    ...
    function onSideGroupAction(sideGroup: NSideGroup, action: SideAction) {
        log("sidegroup-action", {
            sideId: props.side.id,
            sideGroup: { id: sideGroup.id, name: (sideGroup as any).name },
            action,
        });
    ...
    const onUnitAction = (unit: NUnit, action: UnitAction) => {
        // 👇 if you never see this log when clicking "Duplicate" from ORBAT, the bug is inside OrbatSideGroup/unit-row menu
        log("unit-action received", {
            sideId: props.side.id,
            unit: miniUnit(unit),
            action,
        });
...
<template>
    <div class="pl-4">
        <header ref="dropRef"
            ...
            <IconDrag class="size-6 flex-none cursor-move text-gray-500 group-focus-within:opacity-100 group-hover:opacity-100 sm:-ml-3 sm:opacity-0"
                      ref="dragRef" />
            ...
            <button @click="toggleOpen"
                ...
                <ChevronUpIcon :class="isOpen ? 'rotate-180 transform' : ''"
            ...
            <SideDropdownMenu @action="onSideAction"
                              :is-locked="isLocked"
                              :is-hidden="isHidden" />
            <TreeDropIndicator v-if="instruction" :instruction="instruction" class="z-10" />
        </header>
        <EditSideForm v-if="showEditSideForm"
                      :side-id="side.id"
